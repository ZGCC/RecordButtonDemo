...
+ (RecordHUD *)shareView{
    ...
    static RecordHUD *shareView;
    
    dispatch_once(&once, ^{
        shareView = [[RecordHUD alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
+ (void)setTitle:(NSString*)title afterDelay:(NSTimeInterval)delay{
    NSLog(@"---%s", __FUNCTION__);
    [[RecordHUD shareView] setTitle:title];
    [[RecordHUD shareView] hideAfterDelay:delay];
}

- (void)hideAfterDelay:(NSTimeInterval)delay{
    NSLog(@"---%s", __FUNCTION__);
    [self performSelector:@selector(dismissAfterDelay) withObject:nil afterDelay:delay];
}
